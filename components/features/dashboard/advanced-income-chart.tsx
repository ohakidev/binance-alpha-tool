...
export function AdvancedIncomeChart() {
  ...
  const stats = useMemo(() => {
    ...
    const trendPercentage =
      ((data[data.length - 1].profit - data[0].profit) / data[0].profit) * 100;
              ...
              {data.map((item, index) => (
                ...
                >
                  <div className="relative w-full flex items-end justify-center gap-1 h-full">
                    ...
                    >
                      <motion.div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity rounded-t-lg" />
                      {/* Tooltip */}
